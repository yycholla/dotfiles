...
setenv -h log_success "#[fg=cyan,italics] [success]#[default]#[italics]"

#
# Menus
#

set -g @wk_menu_resurrectsaverestore \
'"Save session (Ctrl-a Ctrl-s)" "s" save-session \
"Restore session (Ctrl-a Ctrl-r)" "r" restore-session \
"" \
"Manual save" "S" "run-shell \"$HOME/.tmux/plugins/tmux-resurrect/scripts/save.sh\"" \
"Manual restore" "R" "run-shell \"$HOME/.tmux/plugins/tmux-resurrect/scripts/restore.sh\""'

set -g @wk_menu_copypaste \
'"Enter copy mode (vi)" "c" copy-mode \
"Paste buffer" "p" pasteb \
...
set -g @wk_menu_sessionresurrect \
'"Save session (Ctrl-s)" "s" save-session \
"Restore session (Ctrl-r)" "r" restore-session'
...
"" \
"+Session (Resurrect)" "S" "show-wk-menu #{@wk_menu_sessionresurrect}" \
Detach "D" detach \
Suspend "Z" suspendc \
"" \
"Reload config (Ctrl-a r)" "r" reload-config \
Customize "," "customize-mode -Z"'

set -g @wk_menu_helpkeys \
'"List all keys" "k" "list-keys -N" \
"Show keybinding hints" "?" "display-message \"Ctrl+Space: Which-key menu | Ctrl+a r: Reload | Ctrl+a Ctrl+s: Save session | Ctrl+a Ctrl+r: Restore | Alt+Arrows: Navigate panes\"" \
"Show options" "o" "show-options -g" \
"Show environment" "e" "showenv -g" \
"tmux version" "v" "display-message \"tmux #{version}\""'

set -g @wk_menu_root \
'"Run command" "space" command-prompt \
"Last window" "tab" last-window \
"Last pane" "`" last-pane \
"" \
"+Resurrect (Save/Restore)" "r" "show-wk-menu #{@wk_menu_resurrectsaverestore}" \
"+Copy/Paste" "c" "show-wk-menu #{@wk_menu_copypaste}" \
"" \
...
"+Help/Keys" "?" "show-wk-menu #{@wk_menu_helpkeys}"'
...
display "#{log_success} Config reloaded!"'

set -gF command-alias[203] restart-pane=\
'display "#{log_info} Restarting pane" ; \
respawnp -k -c #{pane_current_path}'

set -gF command-alias[204] save-session=\
'display "#{log_info} Saving session..." ; \
run-shell "$HOME/.tmux/plugins/tmux-resurrect/scripts/save.sh" ; \
display "#{log_success} Session saved!"'

set -gF command-alias[205] restore-session=\
'display "#{log_info} Restoring session..." ; \
run-shell "$HOME/.tmux/plugins/tmux-resurrect/scripts/restore.sh" ; \
display "#{log_success} Session restored!"'
